...
def get_course_info(db: Session = Depends(get_db), current_user=Depends(get_current_user)):
    message = get_course_info_from_db(current_user=current_user, db=db)
    return message


@course_router.delete("/course/{id}")
def delete_course(*, id, db: Session = Depends(get_db), current_user=Depends(get_current_user)):
