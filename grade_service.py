...
def assign_grade_to_student_in_db(data: GetId,student_id: int,grade: str,db: Session,current_user: User = Depends(get_current_user),):
...
def get_gpa_by_student_id_from_db(*, student_id: int, db: Session, current_user: User = Depends(get_current_user)):
    current_user_in_db = (db.query(User).filter(User.username == current_user["sub"]).first())
...
def update_grade_in_db(student_id: int,grade: str,data: GetId,db: Session,current_user: User = Depends(get_current_user),):
    current_user_in_db = (db.query(User).filter(User.username == current_user["sub"]).first())
    ...
    return {
        "message": "Grade is added"
    }


def delete_grade_in_db(data: GetId,student_id: int,db: Session,current_user: User = Depends(get_current_user)):
    current_user_in_db = (db.query(User).filter(User.username == current_user["sub"]).first())
